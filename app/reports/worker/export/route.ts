...
export async function GET(request: NextRequest) {
  ...
  if (mode === 'day') {
    ...
    return new Response(csv, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="worker-report-${dateKey}.csv"`,
      },
    });
  }
  ...
  const workerId = sp.get('workerId') && agg.byWorker.has(sp.get('workerId')!) ? sp.get('workerId')! : workerIds[0];
  const detail = rows.filter((r) => r.workerId === workerId);
  const workerName = workerId ? agg.byWorker.get(workerId)!.name : 'worker';
  ...
  return new Response(csv, {
    headers: {
      'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': `attachment; filename="worker-report-${safeName}-${year}-${String(month).padStart(2, '0')}.csv"`,
    },
  });
}
