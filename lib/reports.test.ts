...
describe('aggregate', () => {
  ...
  it('totals what each company owes — exactly what its workers were paid', () => {
    const r = aggregate(rows);
    expect(r.byCompany.get('c1')!.pay.toString()).toBe('1250'); // 750 + 500
    expect(r.byCompany.get('c2')!.pay.toString()).toBe('600');
  });
  ...
  it('derives the same grand total from workers and from companies', () => {
    const r = aggregate(rows);
    const viaWorkers = [...r.byWorker.values()].reduce((a, b) => a.plus(b.pay), d(0));
    const viaCompanies = [...r.byCompany.values()].reduce((a, b) => a.plus(b.pay), d(0));
    expect(viaWorkers.equals(viaCompanies)).toBe(true);
    expect(viaWorkers.equals(r.totals.pay)).toBe(true);
  });
  ...
  it('uses the snapshot rate it is given, not any current rate', () => {
